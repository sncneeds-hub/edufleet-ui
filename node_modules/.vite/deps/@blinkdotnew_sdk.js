...
var BlinkAuth = class {
  ...
  setupParentWindowListener() {
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data?.type === "BLINK_AUTH_LOGOUT") {
        console.log("📤 Received logout command from parent window");
        this.clearTokens();
      }
    });
   ...
   */
  async initialize() {
    console.log("🚀 Initializing Blink Auth...");
    this.setLoading(true);
    try {
      this.migrateExistingTokens();
      if (this.isIframe) {
        console.log("🔍 Detected iframe environment, waiting for parent tokens...");
      ...
      if (tokensFromUrl) {
        console.log("📥 Found tokens in URL, setting them...");
      ...
      if (storedTokens) {
        ...
        console.log("🔧 Tokens set in auth state, refresh token available:", !!this.authState.tokens?.refresh_token);
  ...
  async recoverAuthState() {
    console.log("🔄 Recovering auth state...");
    this.clearTokens();
  ...
  async validateStoredTokens(tokens) {
    try {
      ...
      if (response.ok) {
        ...
        console.log("🔄 Access token expired (server validation), attempting refresh...");
        if (this.isRefreshTokenExpired()) {
  ...
  async setTokens(tokens, persist) {
    ...
    if (persist && typeof window !== "undefined") {
      try {
        ...
        console.log("💥 Error persisting tokens to localStorage:", error);
        if (error instanceof DOMException && error.name === "SecurityError") {
          console.log("🚫 localStorage access blocked - running in cross-origin iframe");
        }
      }
    }
    let user = null;
  ...
  clearTokens() {
    if (typeof window !== "undefined") {
      try {
        ...
        console.log("💥 Error clearing tokens from localStorage:", error);
      }
    }
    this.updateAuthState({
      user: null,
  ...
  getStoredTokens() {
    ...
    try {
      ...
      if (stored) {
        ...
        console.log("📦 Parsed stored tokens:", {
      ...
      if (error instanceof DOMException && error.name === "SecurityError") {
        console.log("🚫 localStorage access blocked - likely due to cross-origin iframe restrictions");
      }
      return null;
    }
  }
  ...
  clearUrlTokens() {
    ...
    console.log("🧹 URL cleaned up, removed auth parameters");
  }
  redirectToAuth() {
    if (typeof window !== "undefined") {
      this.login();
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.websocket.onopen = () => {
          console.log(`🔗 Connected to realtime for project ${this.projectId}`);
          this.isConnecting = false;
          this.isConnected = true;
        ...
        this.websocket.onclose = () => {
          console.log(`🔌 Disconnected from realtime for project ${this.projectId}`);
          this.isConnecting = false;
          this.isConnected = false;
...
//# sourceMappingURL=@blinkdotnew_sdk.js.map
